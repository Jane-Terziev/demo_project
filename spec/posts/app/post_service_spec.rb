RSpec.describe Posts::App::PostService, type: :unit do
  subject(:service) do
    described_class.new(
  ...
  let(:post_repository) { class_double('Posts::Domain::Post') }
  let(:event_publisher) { instance_double('MetadataEventPublisher').as_null_object }

  describe "#.create_post(command)" do
    let(:command) { Posts::Domain::Post::CreateCommand.new(title: 'title', body: 'body') }
  ...
  describe "#.update_post(command)" do
    let(:command) { Posts::Domain::Post::UpdateCommand.new(id: 1, title: 'title', body: 'body') }

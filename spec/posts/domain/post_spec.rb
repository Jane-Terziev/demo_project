...
RSpec.describe Posts::Domain::Post, type: :model do
  describe "#.create_new(title:, body:)" do
    context "when creating a new post" do
      it 'should assign all attributes' do
        title = 'title'
        body = 'body'
        post = Posts::Domain::Post.create_new(title: title, body: body)
        expect(post.title).to eq(title)
        expect(post.body).to eq(body)

        expect { Posts::Domain::Post.save!(post) }.to_not raise_error

        expect(post.archived).to eq(false)
        expect(post.domain_events).to contain_exactly(kind_of(Posts::Domain::Post::CreatedEvent))
      end
    end
  end
  ...
  describe "#.update_post(title:, body:)" do
    context "when updating an archived post" do
      it 'should raise PostArchivedError' do
        ...
        expect { post.update_post(title: title, body: body)}.to raise_error(Posts::Domain::Post::ArchivedError)
      end
    end
    ...
    context "when updating a post that is not archived" do
      it 'should update attributes' do
        ...
        expect(post.domain_events).to contain_exactly(kind_of(Posts::Domain::Post::UpdatedEvent))
      end
    end
  end
  ...
  describe "#.archive" do
    context "when post is not archived" do
      it 'should change archived to true' do
        ...
        expect(post.domain_events).to contain_exactly(kind_of(Posts::Domain::Post::ArchivedEvent))
      end
    end
  ...
  describe '#.add_comment(title:, body:)' do
    context "when the post has been archived" do
      it 'should raise ArchivedError' do
        post = build(:post, archived: true)
        expect { post.add_comment(title: 'new title', body: 'new body') }.to raise_error(Posts::Domain::Post::ArchivedError)
      end
    end
    ...
    context "when the post is not archived" do
      it 'should raise ArchivedError' do
        ...
        expect(post.domain_events).to contain_exactly(kind_of(Posts::Domain::Comment::CreatedEvent))
      end
    end
  end
end
